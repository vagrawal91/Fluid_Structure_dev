...
program flutas
  ...
  use mpi
  use decomp_2d
  use mod_bound     , only: boundp,updt_rhs_b,bounduvw, inflow
  ...
  !@cuf use cudafor
	...
	!v------------------------------------------------------
#if defined(_IBM)
	use mod_param_fibm,       only: solidity, np, nl, nno
	use mod_common_fibm,      only: forcex, forcey, forcez !, & 
	use mod_common_fibm,      only: ap, pmax
	use mod_loadpart,         only: loadpart   
	use initparticles_fibm,   only: initparticles
	use mod_kernel,           only: kerneltest
	use mod_Update_Pos,       only: Update_Pos
	use mod_source,           only: add_ibm_force
#endif
  !
  !$ use omp_lib
  !
  ...
  real(rp), dimension(:,:), allocatable :: vel2d
  !
#if defined(_HEAT_TRANSFER)
  ...
  real(rp), allocatable, dimension(:)   :: ap_p,bp_p,cp_p
  real(rp) :: normfftp
  ! 
  ...
  integer :: ng1, ng2, ng3
  !
	real(rp) :: a1,a2,a3, b1,b2,b3, gm1,gm2,gm3, time_s
#if defined(_IBM)
  real                            :: sumk  
	integer                         :: pp, kkv, ll, istepv !
	!real, dimension(0:i1,0:j1,0:k1):: durkold,dvrkold,dwrkold   ! check if really needed
  real                            :: v_bulk_all, norm, norm_all
	real                            :: comp_time,comp_time_avrg,comp_time_min,comp_time_max ! for mpi timer
	real,dimension(np*nl,6)         :: fiber_inf
	character (len=21)              :: itoa   !v in writing fiber coords, in Akshay syntaxes
  !character(10)                   :: filename    !v not used
	character*7 number
  ...
  allocate(ap_p(n_z(3)),bp_p(n_z(3)),cp_p(n_z(3)))
  ...
  if(.not.restart) then
    ...
    !$acc end kernels
  	...
  	enddo
		!
		if (myid .eq. 0) print*,'Solid volume fraction of particles = ', solidity !write(6,*)
		if (istep==0) then
			do pp=1,pmax
    ...
    do pp=1,pmax
    if (ap(pp)%mslv .ge. 0) then
      open(42,file='data/Filament_coords_restart'//'.txt')
      do j=1,nno
        write(42,'(3E15.7)') ap(pp)%xll(j),ap(pp)%yll(j),ap(pp)%zll(j)
      enddo
      close(42)
  ...
  call initsolver(n,dims,dims_xyz(:,3),dli,nh_d,dzci_g,dzfi_g,cbcpre,bcpre(:,:),(/'c','c','c'/),lambdaxyp, & 
                  ap_p,bp_p,cp_p,arrplanp,normfftp,rhsbp_x,rhsbp_y,rhsbp_z)
  ...
  do while(.not.is_done)
    ...
    do irk_ss = 1,n_stage ! n_stage=1 for AB2, n_stage=3 for RK3
...
#if defined(_IBM)
			!
      call profiler_start("IBM_T", tag = .true., tag_color = COLOR_RED)
			!
			call Update_Pos(u,v,w, f_t12, istep, time, forcex, forcey, forcez)

			call boundp(cbcpre,n,bcpre,nh_d,nh_u,halo_p,dl,dzc,dzf,forcex)
      call boundp(cbcpre,n,bcpre,nh_d,nh_u,halo_p,dl,dzc,dzf,forcey)
      call boundp(cbcpre,n,bcpre,nh_d,nh_u,halo_p,dl,dzc,dzf,forcez)

      call add_ibm_force(n(1),n(2),n(3),f_t12,nh_u,1._rp/rho_sp,u,v,w,forcex,forcey,forcez)
      !
			call profiler_stop("IBM_T")
			!
#endif
      ...
      call solver_gpu(n_z,dims_xyz(:,3),arrplanp,normfftp,lambdaxyp,ap_p,bp_p,cp_p,cbcpre(:,:),(/'c','c','c'/),p)
#else
      
			call solver_cpu(n,arrplanp,normfftp,lambdaxyp,ap_p,bp_p,cp_p,cbcpre(:,3),(/'c','c','c'/),p)
      ...
      call bounduvw(cbcvel,n,bcvel,nh_d,nh_u,halo_u,is_outflow,dl,dzc,dzf,u,v,w)
			!call inflow(n1,n2,n3,+2,nh_u,.false.,vel2d,u,v,w)
      !
#if defined(_OPENACC)
    ...
    if ( (mod(istep,isave) .eq. 0) .or. is_done .and. (.not.kill) ) then
      ...
      !
			! Save Fiber data here
			!
			!
#if defined(_IBM)
			if (np/=0)    call loadpart(1,istep)
#endif			
			!
#if defined(_HEAT_TRANSFER)
      ...
      !
    endif  !v if istep == isave
#endif
    !
#if defined(_IBM)

	if (mod(istep,iout1d).eq.0) then !iout0d
    do pp=1,pmax
      if (ap(pp)%mslv>0) then

        write(number,'(i3.3)') ap(pp)%mslv
        open(59,file='data/last_point'//trim(number)//'.out',position='append')
        write(59,'(4E16.8)') time,ap(pp)%yll(nno),ap(pp)%zll(nno),ap(pp)%xll(nno)
        close(59)

        !open(60,file=datadir//'last_point_vel'//number//'.txt',position='append')
        !write(60,'(2E16.8)') time,ap(pp)%vl(NL)
        !close(60)
  ...
  if (mod(istep,iout1d).eq.0) then
    do pp=1,pmax
      if (ap(pp)%mslv>0) then
        ...
        open(42,file='data/Filament'//trim(number)//'step'//trim(filenumber)//'.out')
        do j=1,nno
					write(42,'(3E15.7)') ap(pp)%xll(j),ap(pp)%yll(j),ap(pp)%zll(j)
					!write(42,'(7E15.6)')  ap(pp)%xll(j),ap(pp)%yll(j),ap(pp)%zll(j),ap(pp)%qN1(j),ap(pp)%qN2(j),ap(pp)%qN3(j),ap(pp)%qN4(j)
        enddo
        close(42)

        open(59,file='data/mpnt_coord'//trim(number)//'.txt',position='append')
        write(59,'(4E15.7)') time, ap(pp)%xll(int((nno+1)/2.)), &
		   								ap(pp)%yll(int((nno+1)/2.)), ap(pp)%zll(int((nno+1)/2.))
        close(59)


        open(60,file='data/mpnt_vel'//trim(number)//'.txt',position='append')
        write(60,'(4E15.7)') time, ap(pp)%dxdt(int((nno+1)/2.)), &
											ap(pp)%dydt(int((nno+1)/2.)),ap(pp)%dzdt(int((nno+1)/2.))
        close(60)
        
				open(61,file='data/mpnt_acc'//trim(number)//'.txt',position='append')
        write(61,'(4E15.7)') time, ap(pp)%ua(int((nno+1)/2.)), &
											ap(pp)%va(int((nno+1)/2.)),ap(pp)%wa(int((nno+1)/2.))
        close(61)
