...
module mod_bound
  ...
  !
  private
  public  :: bounduvw, boundp, updt_rhs_b, updthalo, inflow
  !
  contains
  !
  ...
  !
  ! TODO: add GPU here?
  subroutine inflow(nx,ny,nz,idir,nh_u,is_correc,vel2d,u,v,w)
    !
    use mod_common_mpi, only: MPI_PROC_NULL,left,right,front,back,top,bottom
    !
    implicit none
    ...
    real(rp), intent(in   ), dimension(     1:,    1:)          :: vel2d
    real(rp), intent(inout), dimension(1-nh_u:,1-nh_u:,1-nh_u:) :: u,v,w
    !
    integer :: i,j,k,q
    !
