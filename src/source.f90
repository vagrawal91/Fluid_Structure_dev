...
module mod_source
  ...
  !
#if defined(_IBM)
  subroutine add_ibm_force(nx,ny,nz,f_t12,nh_u,rho0i,u,v,w,forcex,forcey,forcez)
    !
    implicit none 
    !
    integer , intent(in   )                                     :: nx,ny,nz
    ...
    real(rp), intent(in   )                                     :: rho0i
    real(rp), intent(inout), dimension(1-nh_u:,1-nh_u:,1-nh_u:) :: u,v,w
    real(rp), intent(in   ), dimension(     0:,     0:,     0:) :: forcex,forcey,forcez
    !
    integer :: i,j,k
    !
    ...
    do k=1,nz
      do j=1,ny
        do i=1,nx
          !
          u(i,j,k) = u(i,j,k) + f_t12*( forcex(i,j,k) )!*rho0i
          v(i,j,k) = v(i,j,k) + f_t12*( forcey(i,j,k) )!*rho0i
          w(i,j,k) = w(i,j,k) + f_t12*( forcez(i,j,k) )!*rho0i
          !
        enddo
      enddo
    enddo
  ...
  subroutine bulk_forcing_src(forcing_type,is_forced,nx,ny,nz,dx,dy,dz,f_t12, &
                              nh_d,nh_u,dzc,dzf,rho,u,v,w,f,time_s)
    !
    ...
    use mod_param, only: lx,ly,lz,bvel_x,bvel_y,bvel_z,dpdl_x,dpdl_y,dpdl_z, pi
#if !defined(_USE_VOF)
    ...
    integer         , intent(in   )                                     :: nx,ny,nz
    real(rp)        , intent(in   )                                     :: dx,dy,dz
    real(rp)        , intent(in   )                                     :: f_t12, time_s
    integer         , intent(in   )                                     :: nh_d,nh_u
          ...
          do j=1,ny
            do i=1,nx
              v(i,j,k) = v(i,j,k) + (bvel_y-meanvel)
							!v(i,j,k) = v(i,j,k) + (bvel_y*1.0*sin(2.*pi*time_s) - meanvel)
							!v(i,j,k) = v(i,j,k) + (bvel_y*cos(2.*pi*(1./40.)*time_s) - meanvel)
            enddo
          enddo
        enddo
        !$acc end kernels
