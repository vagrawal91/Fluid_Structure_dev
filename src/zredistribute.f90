...
call MPI_Type_get_extent(MPI_REAL8, lb, ereal8, ierr)

call MPI_Type_contiguous(imax, MPI_REAL8, mytype1, ierr)
call MPI_Type_commit(mytype1, ierr)

call MPI_Type_create_hvector(jmax, 1, size(p,1)*ereal8, mytype1, mytype2, ierr)
call MPI_Type_commit(mytype2, ierr)

call MPI_Type_create_hvector(ksol, 1, size(p,1)*size(p,2)*ereal8, mytype2, mytype3, ierr)
call MPI_Type_commit(mytype3, ierr)

! construct a derived type (montype3) for handling 
!            p1(ib:ib+imax-1,jb:jb+jmax-1,:)

call MPI_Type_contiguous(imax, MPI_REAL8, montype1, ierr)
call MPI_Type_commit(montype1, ierr)

call MPI_Type_create_hvector(jmax, 1, size(p1,1)*ereal8, montype1, montype2, ierr)
call MPI_Type_commit(montype2, ierr)

call MPI_Type_create_hvector(ksol, 1, size(p1,1)*size(p1,2)*ereal8, montype2, montype3, ierr)
call MPI_Type_commit(montype3, ierr)
...
call MPI_Type_free(mytype1, ierr)
call MPI_Type_free(mytype2, ierr)
call MPI_Type_free(mytype3, ierr)
call MPI_Type_free(montype1, ierr)
call MPI_Type_free(montype2, ierr)
call MPI_Type_free(montype3, ierr)
...
call MPI_Get_address( p(1,1,1), base,ierr)
call MPI_Get_address(p1(1,1,1),base1,ierr)

do j = 0, dims(2) - 1
  do i = 0, dims(1) - 1
    call MPI_Cart_rank(comm_cart,(/i,j/),sub_rank,ierr)
    kb     = (i*dims(2)+j)*ksol + 1
    call MPI_Get_address(p(1,1,kb), address, ierr)
    disp(sub_rank+1) = address - base
    ib = i*imax + 1
    jb = j*jmax + 1
    call MPI_Get_address(p1(ib,jb,1), address, ierr)
...
if(in .eq. 0) then
  call MPI_Alltoallw(p(1,1,1),counts,disp, types, &
                    p1(1,1,1),counts,disp1,types1,comm_cart,ierr)
endif

if(in .eq. 1) then
  call MPI_Alltoallw(p1(1,1,1),counts,disp1,types1, &
                      p(1,1,1),counts,disp, types,comm_cart,ierr)
endif
...
if (in.eq.0) then
  do j = 0,dims(2)-1
    do i = 0,dims(1)-1
      sub_coords(1) = i
      sub_coords(2) = j
      call MPI_Cart_rank(comm_cart,sub_coords,sub_rank,ierr)
      kb     = (i*dims(2)+j)*ksol + 1
